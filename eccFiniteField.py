...
class EllipticCurve:
    ...
    def addition(self, P1, P2):
        ...
        if self.equalModp(x1, x2) and self.equalModp(y1, y2):
            lamda = self.reduceModp((3 * x1 * x1 + self.a) * self.inverseModp(2 * y1))
        else:
            lamda = self.reduceModp((y1 - y2) * self.inverseModp(x1 - x2))

        v = self.reduceModp(y1 - lamda * x1)
        x3 = self.reduceModp(lamda * lamda - x1 - x2)
        y3 = self.reduceModp(-lamda * x3 - v)
    ...
    # helper functions
    # reduce to integer mod p
    def reduceModp(self, x):
        return x % self.p

    # check if there difference is divisible by mod p
...
'''
ec_test = EllipticCurve(2,7,19)
ec_test.printPoints()
print(ec_test.numberPoints())
print(ec_test.discriminant())
'''
p = 7
count = 0
